...
def mask(frame): #masks the frame and returns image which has pointers visible 
	...
	picture = cv2.morphologyEx(mask, cv2.MORPH_OPEN, np.ones((7,7),np.int8),10)
	return picture

def crop(thresh,pts1): #detects the corners of the the keyboard to finally crop the big rectangle
...
def display(pressed,frame,caps,typed): #displays the pressed keys on the frame
	x = ''
	shift = False
